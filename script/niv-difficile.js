...
function loadQuestion() {
    ...
    const currentQuestion = quiz_one_piece.questionsModeDifficile[currentQuestionIndex];
...
function updateProgressBar() {
    const progressBarPercentage = ((currentQuestionIndex + 1) / quiz_one_piece.questionsModeDifficile.length) * 100;
    ...
    if (currentQuestionIndex + 1 === quiz_one_piece.questionsModeDifficile.length) {
...
nextButton.addEventListener("click", () => {
    ...
    if (currentQuestionIndex < quiz_one_piece.questionsModeDifficile.length) {
...
function showFinalResult() {
    questionText.textContent = `Tu as obtenu ${score}/${quiz_one_piece.questionsModeDifficile.length}. Tentatives restantes : ${attemptsLeft}`;
    updateProgressBar();
    let message = "";

    // Message personnalisé en fonction du score
    if (score === quiz_one_piece.questionsModeDifficile.length) {
        message = "Excellent ! Mais tu es encore loin du One Piece ! 💀";
    } else if (score >= quiz_one_piece.questionsModeDifficile.length / 2) {
